<script>
  ...
  const resizeEvent = ()=> {
    ...
    dimensions.upsideRegion = dimensions.height * 0.1;
    dimensions.downsideRegion = dimensions.height * 0.9;
    dimensions.leftsideRegion = dimensions.width * 0.2;
    dimensions.rightsideRegion = dimensions.width * 0.8;
  }
  ...
  const highlighting = (e) => {
    ...
    // console.log(e.layerX, e.layerY, item)
...
<div 
  ...
  on:dragend = {e=>{dropEvent(e); console.log('item drop')}}></div>
<div
  class="container"
  on:dragover|stopPropagation = {e=>{highlighting(e)}}
  on:dragleave|stopPropagation = {e=>{classState = "idle"}}>
  <p>
